...
esp_err_t icm20948_read_measurements(data_t *p_dev)
{
    ...
    accel_x =
        (((float)accel_x_raw / 16384.0f) * 9.80665f) + ax_offset; // Digital value -> g -> m/s2
...
esp_err_t KALMAN_readFunction(data_t *p_dev, float *accel_x, float *accel_y, float *accel_z,
                              float *gyro_x, float *gyro_y, float *gyro_z, float ax_offset,
                              float ay_offset, float az_offset, float gx_offset, float gy_offset,
                              float gz_offset)
{
    // Required variables
    int16_t accel_x_raw, accel_y_raw, accel_z_raw;
    ...
    *accel_z = (((float)accel_z_raw / 16384.0f) * 9.80665f) + az_offset;
    ...
    gyro_x_raw = get_raw_axis_data(p_dev, REG_GYRO_X_H, REG_GYRO_X_L);
    *gyro_x =
        -((float)gyro_x_raw / 131.0f) + gx_offset; // Digital value -> dps (degrees per second)
...
float KALMAN_offsets(data_t *p_dev, float *accel_x, float *accel_y, float *accel_z, float *gyro_x,
                     float *gyro_y, float *gyro_z, float *ax_offset, float *ay_offset,
                     float *az_offset, float *gx_offset, float *gy_offset, float *gz_offset)
{
    // Required variables
    int16_t accel_x_raw, accel_y_raw, accel_z_raw;
    int16_t gyro_x_raw, gyro_y_raw, gyro_z_raw;
    ...
    // Accel: X
    accel_x_raw = get_raw_axis_data(p_dev, REG_ACCEL_X_H, REG_ACCEL_X_L);
    *accel_x = (((float)accel_x_raw / 16384.0f) * 9.80665f); // Digital value -> g -> m/s2

    // Accel: Y
    accel_y_raw = get_raw_axis_data(p_dev, REG_ACCEL_Y_H, REG_ACCEL_Y_L);
    *accel_y = -(((float)accel_y_raw / 16384.0f) * 9.80665f);

    // Accel: Z
    accel_z_raw = get_raw_axis_data(p_dev, REG_ACCEL_Z_H, REG_ACCEL_Z_L);
    *accel_z = (((float)accel_z_raw / 16384.0f) * 9.80665f);

    // Gyro: X
    gyro_x_raw = get_raw_axis_data(p_dev, REG_GYRO_X_H, REG_GYRO_X_L);
    *gyro_x = -((float)gyro_x_raw / 131.0f); // Digital value -> dps (degrees per second)
    ...
    *ax_offset = -(*accel_x);
    *ay_offset = -(*accel_y);
    *az_offset = 9.81f - (*accel_z);
    *gx_offset = -(*gyro_x);
    *gy_offset = -(*gyro_y);
    *gz_offset = -(*gyro_z);

    // ESP_LOGE(TAG, "A_o: x - %.2f, y - %.2f, z - %.2f | G_o: x - %.2f, y - %.2f, z - %.2f",
