...
```
  ...
  ║  logo.svg · banner.png · bannerPixelart.png                 ║
  ║  version.json · manifest.sha256  (generated at deploy)     ║
  ╚════════════════════════════════════════════════════════════╝
```
...
├── AUDIT.md             # Privacy & integrity verification guide
└── html/
...
The following files are **not in the repository** — they are generated by the CI/CD pipeline
at deploy time and written into `html/` on the VPS:

| File | Contents |
|---|---|
| `version.json` | Commit SHA, branch, deploy timestamp, and `files_hash` (SHA-256 over source files) |
| `manifest.sha256` | SHA-256 over all deployed files including `version.json` |

The `website/` folder in this repository is the **source of truth** for all source files.
